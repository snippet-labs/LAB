    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center justify-center px-6 md:px-16 py-12 gap-12">
        <div className="text-white max-w-4xl w-full">
          ...
          >
            <div className="flex overflow-hidden border p-1 mt-15 EDGEGRADIENT">
              <motion.div
                initial={{ x: 0 }}
                animate={{ x: '-100%' }}
                transition={{ duration: 35, repeat: Infinity, ease: 'linear' }}
                className="flex flex-shrink-0 gap-1 EDGEGRADIENT"
              >
                {stackData.map((image, index) => {
                  return (
                    <img
                      src={image}
                      key={index}
                      className="w-15 h-15 md:w-20 lg:w-25 md:h-20 lg:h-25  group bg-white/5 backdrop-blur-md border border-neutral-800 rounded-2xl shadow-xl"
                    />
                  );
                })}
              </motion.div>
